...
void QVPNClientConnection::socket_readyRead()
{
    ...
    while (mReceiveBuffer.contains('\n')) {
        ...
        case cmdStart: {
            ...
            mServerId = jObj.value("serverid").toInt();
            mHostname = jObj.value("server").toString().remove("\n").remove("\r");
            mPort = jObj.value("port").toString().remove("\n").remove("\r");
            mTemplateUrl = jObj.value("template").toString().remove("\n").remove("\r");
            mLocalPort = jObj.value("localport").toString().remove("\n").remove("\r");
            mTcpOrUdp = jObj.value("tcporudp").toString().remove("\n").remove("\r");

            bool fixDNS = jObj.value("fixDNS").toBool();
            QString dns1 = jObj.value("dns1").toString().remove("\n").remove("\r");
            QString dns2 = jObj.value("dns2").toString().remove("\n").remove("\r");
