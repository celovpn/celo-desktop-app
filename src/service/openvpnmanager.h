...
private slots:
    ...
    void getTemplateError(QNetworkReply::NetworkError error);
    void getTemplateFinished();
...
private:
    ...
    // Hash function for server ids and ports to ids in mOvpnTemplate map
    int serverKey(int serverId, int port);
    ...
    vpnState mState;
    QTimer *mStateTimer;
    int mPID;		// for running process (when running celovpn after crash)
    ...
    // When filtering template for block-outside-dns etc.
    // set this if the template contains DNS settings.
    // If it contains dns settings, don't use user's dns servers
    // or the defaults
    bool mTemplateHasDNSOptions;

    // Map of server id *10 + port to config template
    QMap<int, QByteArray> mOvpnTemplate;
};
