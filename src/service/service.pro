...
lessThan(QT_MAJOR_VERSION, 5) | lessThan(QT_MINOR_VERSION, 6) : error(CeloVPN requires Qt 5.6 or newer but Qt $$[QT_VERSION] was detected.)

TEMPLATE = app

CONFIG(debug, debug|release)::TARGET = celovpnservice
CONFIG(release, debug|release)::TARGET = celovpnservice
...
win32 {
    ...
    QMAKE_POST_LINK = "$$quote($$OUT_PWD_WIN\\..\\fixbinaryadmin.bat) $$quote($$OUT_PWD_WIN\\..\\celovpnservice.exe) $$quote($$OUT_PWD_WIN\\..\\runasadmin.manifest)"
...
macx: {
    ...
    QMAKE_LFLAGS += -F /System/Library/Frameworks/Security.framework/ -sectcreate __TEXT __info_plist $$PWD/CeloVPNHelper-Info.plist -sectcreate __TEXT __launchd_plist $$PWD/net.celo.CeloVPNHelper.plist
    LIBS += -framework Security -framework Cocoa

    DISTFILES += CeloVPNHelper-Info.plist \
                 net.celo.CeloVPNHelper.plist

    # Bundle identifier for your application
    BUNDLEID = $${TARGET}

    HOSTAPP_IDENTIFIER = net.celo.CeloVPN
    # QMAKE_PRE_LINK += /usr/libexec/PlistBuddy -c \'Set :SMAuthorizedClients:0 'anchor apple generic and identifier \\\"$${HOSTAPP_IDENTIFIER}\\\" and (certificate leaf[field.1.2.840.113635.100.6.1.9] /* exists */ or certificate 1[field.1.2.840.113635.100.6.2.6] /* exists */ and certificate leaf[field.1.2.840.113635.100.6.1.13] /* exists */ and certificate leaf[subject.OU] = $${CERT_OU})'\' $$PWD/CeloVPNHelper-Info.plist;
