...
// Template url to download before adding remote machine ip and protocol
const QString kTemplateUrl = "https://celo.net/celo-ovpn-temp.ovpn";
...
//        }
    ...
    // Now fetch config to write ovpn file with
    getTemplate();
}
...
//                );
    ...
    int server = serverKey(mServerId, mPort.toInt());

    if (!mOvpnTemplate.contains(server)) {
        Log::serviceLog("No template data to create ovpn file with");
        sendError("No template ovpn found to connect");
        return false;
    }

    mTemplateHasDNSOptions = false;

    QString templateLines = mOvpnTemplate.value(server);
    QStringList lines = templateLines.split("\n");
    QString filteredLines;
    for (int i = 0; i < lines.count(); ++i) {
        if (lines.at(i).contains("DNS"))
            mTemplateHasDNSOptions = true;

        filteredLines += lines.at(i) + "\n";
    }

    if (!mTemplateHasDNSOptions) {
        if (mFixDNS) {
        ...
        if (!mDNS1.isEmpty()) {
            ff.write(QString("push \"dhcp-option DNS %1\"\n").arg(mDNS1).toLatin1());
        }
        if (!mDNS2.isEmpty()) {
            ff.write(QString("push \"dhcp-option DNS %1\"\n").arg(mDNS2).toLatin1());
        }
    ...
    ff.write(filteredLines.toLatin1());
...
void OpenvpnManager::getTemplate()
{
    QNetworkAccessManager *nam = new QNetworkAccessManager(this);

    QNetworkRequest request(QUrl::fromUserInput(mTemplateUrl));
    Log::serviceLog("Fetching config from url " + request.url().toString() + " into serverKey " + QString::number(serverKey(mServerId, mPort.toInt())));

    QNetworkReply *reply = nam->get(request);
    reply->setProperty("serverKey", serverKey(mServerId, mPort.toInt()));
    connect(reply, SIGNAL(error(QNetworkReply::NetworkError)),
            this, SLOT(getTemplateError(QNetworkReply::NetworkError)));
    connect(reply, &QNetworkReply::finished,
            this, &OpenvpnManager::getTemplateFinished);
}

void OpenvpnManager::getTemplateError(QNetworkReply::NetworkError error)
{
    QNetworkReply *reply = qobject_cast<QNetworkReply*>(sender());
    int serverKey = reply->property("serverKey").toInt();
    int index = serverKey / 10;
    int portnumber = serverKey % 10;
//    AServer server = AuthManager::instance()->getServer(index);
//    log::logt("Unable to fetch template for server: " + server.name + ": " + QString::number(error));
    emit sendError("Unable to fetch configuration from " + reply->url().toString());
    stop();
}

void OpenvpnManager::getTemplateFinished()
{
    // Got template
    QNetworkReply *reply = qobject_cast<QNetworkReply*>(sender());
...
//    log::logt("Got template for server " + server.name + " with port " + QString::number(portnumber));
    ...
    QByteArray templateBytes = reply->readAll();

//    templateBytes.replace('\r', QString());

    mOvpnTemplate.insert(serverKey, templateBytes);
...
#ifdef Q_OS_LINUX
    args << "--up" << ServicePathHelper::Instance()->upScriptFilename();     // /Applications/Celo VPN.app/Contents/Resources/client.up.celovpn.sh
    args << "--down" << ServicePathHelper::Instance()->downScriptFilename(); // /Applications/Celo VPN.app/Contents/Resources/client.down.celovpn.sh
#endif

#ifdef Q_OS_DARWIN
    args << "--up" << escapeSpaces(ServicePathHelper::Instance()->upScriptFilename()) + " -d -f -m -o -w";
    args << "--down" << escapeSpaces(ServicePathHelper::Instance()->downScriptFilename()) + " -d -f -m -o -w";
#endif
    args << "--up-restart";
...
bool OpenvpnManager::openvpnRunning()
{
    ...
    if (running) {
        Log::serviceLog("openvpn is running because we have a QProcess in running or starting state");
    }

    if (!running) {
        running = mSocket.get() != NULL && mSocket->isOpen();
        if (running)
            Log::serviceLog("openvpn is running because the socket is open");
    }
...
#else
        if (!running) {
            QTemporaryFile file(QDir::tempPath() + "/celovpn-tmp-XXXXXX.sh");
            QTemporaryFile outf(QDir::tempPath() + "/celovpn-tmp-XXXXXX.out");
            if (file.open())
                if (outf.open()) {
                    QString script = QString("ps -xa | grep open | grep execut | grep Celo> ") + outf.fileName();
...
int OpenvpnManager::serverKey(int serverId, int port)
{
    int portnumber = (port == 1194 ? 1 : (port == 53 ? 2 : 3));
    return (serverId * 10) + portnumber;
}
