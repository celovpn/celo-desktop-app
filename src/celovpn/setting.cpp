...
static const QString kDefaultDNS1 = "1.1.1.1";
static const QString kDefaultDNS2 = "84.200.69.80";
...
Setting::Setting()
    :mTesting(false)
{
    mDefaultDNS[0] = kDefaultDNS1;
    mDefaultDNS[1] = kDefaultDNS2;
...
static const QString gs_desktop =
    ...
    "Type=Application\n"
    "Name=celoVPN\n"
    "Exec=/opt/celovpn/celovpn\n"
    "Icon=/usr/share/icons/hicolor/64x64/apps/celovpn.png\n"
    "Comment=OpenVPN client for celo.net\n"
...
void Setting::setStartup(bool v)
{
...
#ifdef Q_OS_WIN
    {
        ...
        static const char * keyname = "CeloVPN";
        if (!v) {
            settings.remove(keyname);
        } else {
            QString val = "\"" + QApplication::applicationFilePath() + "\"";
            val.replace("/","\\");
    ...
    QString pfn = dir + "/net.celo.celovpn.plist";
    QFile pa(pfn);
    if (pa.exists()) {
        if (!pa.remove())
    ...
    if (v) {
        ...
        if (!pa.open(QIODevice::WriteOnly)) {
            ...
            QString s =
                ...
                "<dict>\n"
                "<key>Label</key>\n"
                "<string>net.celo.celovpn</string>\n"
...
void Setting::setServer(int ixsrv)
{
    ...
    Log::logt("Setting::setServer called with index " + QString::number(ixsrv));
    QString newsrv = se.name;
    Log::logt("Setting::setServer server name is " + newsrv);
...
QString Setting::port()
{
    ...
    if (ix > -1 && ix < (int)v_ports.size())
        return QString::number(v_ports.at(ix));
    return QString::number(v_ports.at(0));
}
...
{
    return mSettings.value("e_PrimaryDns", kDefaultDNS1).toString();
}
...
{
    return mSettings.value("e_SecondaryDns", kDefaultDNS2).toString();
}
