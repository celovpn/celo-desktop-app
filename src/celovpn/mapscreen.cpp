...
//    int oldEnc = mEncryption;
    ...
    mShowingNodes = true; // Setting::instance()->showNodes() || mEncryption >= ENCRYPTION_ECCXOR;
    mUseServerColumn = true;

    setRowStyle(mUseServerColumn);

    const QList<int> & coll = AuthManager::instance()->currentEncryptionServers();
...
{
    qDebug() << "Connect button on map screen clicked";
